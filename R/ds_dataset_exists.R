...
#' @return \code{TRUE} if the data-object is available from the datastore given by argument \code{path} or \code{FALSE} else.
#' @export
#'
#' @examples
#' ## not yet
ds_dataset_exists <- function(path, name, version=NULL, verbose=TRUE) {
  ds_isvalid(path)
  ...
  res <- ds_dataset_versions(path, name=name, verbose=verbose)
  if (is.null(res)) {
    return(FALSE)
  }
  if (is.null(version)) {
